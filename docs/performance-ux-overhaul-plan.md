...
## Goal
Make the storefront and dashboard feel instant without rewriting the app. The Django monolith with server-rendered templates is the right architecture for an SEO-dependent single-store brand â€” every problem below is fixable in place. No framework migration, no API layer, no big-bang restyle.
...
| P1 | Background jobs / unblock requests | 1â€“2 days | Checkout & dashboard saves become instant |
| P2 | Cloudinary image delivery | 1 day | Biggest storefront speed win (LCP) |
| P3 | Real caching + query fixes | 1â€“2 days | Faster browse/search/orders pages |
| P4 | Hosting decision | 0.5â€“1 day | Kills cold starts; enables P1/P3 fully |
| P5 | Frontend unification (htmx + CSS) | 3â€“5 days, incremental | SPA-feel cart/filters/wishlist |
| P6 | Code structure & model cleanup | 2â€“3 days, incremental | Developer velocity, fewer bugs |

Phases P0â€“P3 are the core: roughly 4â€“5 days of work for the large majority of the perceptible improvement.

---

## P0 â€” Security & Housekeeping
Status: Done (code) â€” credential rotation still required (see notes)
Depends on: nothing. Do this first; it is half a day.

### 0.1 Rotate leaked database credentials
Real passwords sit in commented-out blocks in `zentanee/settings.py` (Supabase block around line 131, Railway block around line 146). They are in git history permanently.
...
- [ ] Remove the hard-coded fallback `SECRET_KEY` default (line 24) in production paths â€” raise `ImproperlyConfigured` instead when `DEBUG=False` and no key is set.

### 0.2 Remove dead weight
- [ ] Delete `djangorestframework` from `requirements.txt` â€” it is never imported anywhere.
...
- [ ] Deduplicate the Telegram webhook routes registered twice with/without trailing slash in `store/urls.py` (~lines 74â€“79) â€” `APPEND_SLASH` or a single canonical form.
- [ ] Delete `zentanee/v.html` if it is scratch.
...
## P1 â€” Get Blocking Work Out of the Request Path
Status: Done
...
| `store/models.py:254â€“264` | Pillow webp conversion in `save()` | Anyone uploading images (fixed properly in P2) |

### 1.1 Build a minimal outbox queue
No Celery, no Redis broker â€” a plain database table drained by a worker. This survives serverless.
...
- [ ] `signals.py`: replace direct `post_product_to_channel(...)` calls with `enqueue('telegram_product_post', {...})`. Keep the existing `suspend_telegram_autopublish()` semantics â€” suspension should skip the enqueue, not the send.
...
- [ ] Dashboard AI enrichment: enqueue `ai_enrich_draft`; the queue page (`dashboard/ai_queue.html`) polls draft status (it is already AJAX-driven) instead of holding the request open. Show `pending â†’ processing â†’ ready/failed` states on the draft row.
...
## P2 â€” Cloudinary-Native Image Delivery
Status: Done
Depends on: nothing (independent of P1).

We already pay for Cloudinary but convert images to webp ourselves with Pillow inside `Product.save()` (`store/models.py:254â€“264`, also Category/Brand/ProductImages), then serve one fixed file to every device. Cloudinary does format, quality, and resize on the fly via URL parameters â€” better output, zero server CPU.
...
## P3 â€” Real Caching + Query Fixes
Status: Done (code) â€” provision Redis and set REDIS_URL in production
Depends on: P0. Cache backend choice interacts with P4.

The cache backend is `LocMemCache` (`zentanee/settings.py:240`) â€” per-process, so on serverless it caches essentially nothing. Only the nav menu uses it.
...
- [ ] `{% cache %}` fragment around the product-card grid in the `_collection_*` partials, keyed on (filters hash, page, sort). TTL 5â€“10 min.
...
### 3.3 Query fixes
- [ ] `orders()` (`store/views.py:2118â€“2138`): paginate the queryset first, then attach `timeline`/`status_copy` decoration to only the page's rows. Currently the entire order history is materialized per request.
- [ ] Collection pages: `_build_collection_state` runs a price-bounds aggregate (`views.py:1081`), a separate full scan for size options (`views.py:1184` â†’ `1034â€“1047`), a count, and the page query â€” consolidate into one aggregate pass where possible and cache the price-bounds/size-options per (category/brand) for 10 min.
...
## P4 â€” Hosting Decision
Status: Prepared â€” see docs/hosting.md and Procfile; decision/migration pending
...
- [ ] If staying on Vercel is non-negotiable: keep the P1 cron-endpoint trigger and Upstash Redis, and accept cold starts. The rest of the plan still works â€” this phase is an amplifier, not a prerequisite.
...
## P5 â€” Frontend Unification (htmx + one design system)
Status: Done â€” htmx interactions live; zd-* token migration continues page-by-page
Depends on: none. Ship page-by-page; each step is independently deployable.

Two problems: (a) ~10 hand-rolled fetch/JSON endpoints each with bespoke JS for cart, wishlist, and filters; (b) two unrelated design systems â€” `spring-*` (storefront, off `templates/base.html`) and `zd-*` (dashboard, off `templates/dashboard/base.html`) â€” plus heavy inline styling in the dashboard (`dashboard/product_form.html` alone has 18 inline `style=` attributes and an inline `<script>`).

### 5.1 Adopt htmx for interactions (no build step â€” it fits the current stack)
...
- [ ] **Collection filters/sort/pagination:** `hx-get` on the filter form and pager targeting the product-grid partial, with `hx-push-url="true"` so URLs stay shareable/SEO-clean. The partial structure (`_collection_*`) already supports this â€” it is the easy case.
- [ ] **Search suggestions:** replace the hand-rolled typeahead in `base.html:194â€“256` with `hx-trigger="keyup changed delay:300ms"` returning a suggestions partial.
...
## P6 â€” Code Structure & Model Cleanup
Status: Done
...
### 6.2 Model cleanup
- [ ] **Single source of truth for sizes:** drop the CSV `Product.available_sizes` field in favor of `ProductSizeStock` rows (currently both exist, synced by `_sync_size_inventory`, `models.py:190â€“235`). Provide a data migration; keep a read-only property for template compatibility during transition.
...
- [ ] **Guest checkout:** stop creating `guest-<session_key>` rows in `auth_user` (`views.py:1247â€“1253`). Make `Order.user` nullable with `guest_email`/`session_key` fields, or introduce a dedicated `Customer` model. Migrate existing guest users.
- [ ] Fix `Coupon` fields declared with `default=None` on effectively-required columns (`models.py:499â€“504`).
...
- [ ] Keep the existing 28-method `StoreFlowTests` green through every phase â€” it already covers cart guards, checkout, affiliate commissions, and dashboard gating well.
...
- **Rewrite as SPA (Next.js/React) + DRF API** â€” doubles the codebase surface, adds a build pipeline and API versioning for a single-store brand, and sacrifices free server-rendered SEO. None of the current pain comes from the rendering model.
- **Celery + RabbitMQ/Redis broker** â€” overkill for this volume; the outbox table gives durability, retries, and visibility with one model and one command.
- **Tailwind migration** â€” a full-repo restyle for aesthetic parity we can reach by consolidating the existing CSS tokens. Revisit only if a redesign is planned anyway.
- **Microservices split** â€” the image worker is already the one justified external service (GPU workload); everything else belongs in the monolith.

## Sequencing
Recommended order: **P0 â†’ P1 â†’ P2 â†’ P3 â†’ P4 â†’ P5 â†’ P6**, with two notes:
- P4 (hosting) can be pulled forward to before P1 if the migration is approved early â€” it simplifies the worker story from "cron endpoint" to "just a process".
